...
for ll = 1; %:21
        ...
        DampWidth = 0.124;   % At what q-domain are the micelle effects damped. 
        % DampWidth = 0.124 + (ll-11)/250;    
        dDampWidth = 2.2;   % How sharp is the low-q damping of the micelle
        % dDampWidth = 2.2 + (ll-11)/10; 
        BOGcorr = -14.2; 
        % BOGcorr = - 14.2 + (ll-11)/10;    % Correction for deviations from 190 BOG simulation. 
for kk = 1:size(stepnr,2)
...
% To reply to the reviewer they would like a normpdf damping.... 
    LowDamp2 = normpdf(q/(DampWidth)); LowDamp2 = dDampWidth*LowDamp2/(max(LowDamp2));
...
% To reply to the reviewer they would like a normpdf damping....                           
      % dMemVac(kk,:) = (dProtMemVacA(kk,:) + BOGcorr*FlucMicel(2,:) - dProtVac(kk,:)).*LowDamp1;    
       dMemVac(kk,:) = (dProtMemVacA(kk,:) + BOGcorr*FlucMicel(2,:) - dProtVac(kk,:)).*LowDamp2; 
...
hold on; 
plot(q1(FitStart:FitEnd),q1(FitStart:FitEnd).*save2plotagain,'LineWidth',5,'color',[0.8500 0.3250 0.0980]);
plot(q1(FitStart:FitEnd),q1(FitStart:FitEnd).*(Best3(BestIndex,:)),'LineWidth',3,'color',[0.4660 0.6740 0.1880]);   
...
figure('Position', [650 350 500 380])
plot(q,keep2plot,'LineWidth',5,'color',[0.8500 0.3250 0.0980])
hold on
plot(q,LowDamp2,'LineWidth',5,'color',[0.4660 0.6740 0.1880])
