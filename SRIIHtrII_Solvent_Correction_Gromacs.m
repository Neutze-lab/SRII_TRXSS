% close all; clear all; 
filename(1) = ["Data\SrIIHtrII-220BOG-protmem_intensities"]; 
filename(2) = ["Data\SrIIHtrII-220BOG-watercut_intensities"]; 
...
% scalefactor = max(DataProt(3,:))/max(DataWater(2,:)); 
 ...
 scalefactor = 0.8; 
% scalefactor = 0.7423;      % Since the water cage is somewhat too big we need to down-scale the water scattering.
...
% save('SolventExclusionFactorSRIIHtrII.mat','SolventExclusionFactor2');

% For Andrea. Use this to save.... until you have done the pure-water
% Gromacs simulations which Lucija knows. 
% save('SolventExclusionFactorSRIIHtrII.mat','SolventExclusionFactor1');
