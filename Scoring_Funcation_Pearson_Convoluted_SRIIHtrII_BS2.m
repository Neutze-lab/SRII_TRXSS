...
stepnr = [731 731 702 732 703 733 704 734 705 735 706 736 ...    
                ...
                713 743 714 744 715 745 716 746 717 747 718 748 ...
                 ...
                 725 755 726 756 727 757 728 758 729 759 730 760];  
             
% This lists the simulations in the correct order for plotting.                                                                      
corrStart = 84; corrEnd = 425;       % Correlations on protein-only-vaccum curve with data q1(90) = 0.27 Å-1 < q < q1(425) 0 1.00 Å-1. 
...
% Read in experimental data from Richard's analysis.       
        SRIIHtrIIold = load("Data\SRII_HtrII_BS_2.mat");
        Expt1(1,:) = transpose(SRIIHtrIIold.SRII_HtrII_BS_2) + Offset; 
...
for ll = 1:21
         ...
         BestScale2 =   22.4;
         % BestScale2 = 22.4 + (ll-11)/20;  
         DampWidth = 0.278;  % 0.278 At what q-domain are the micelle effects damped. 
         % DampWidth = 0.262  + (ll-11)/250;    
         dDampWidth = 0.8;   % 0.4 How sharp is the low-q damping of the micelle
         % dDampWidth = 0.7 + (ll-11)/100; 
         BOGcorr = -0.4; 
        % BOGcorr = - 0.1 + (ll-11)/20;    % Correction for deviations from 190 BOG simulation. 
for kk = 1:size(stepnr,2)
%% ==========================================================================
...
%% =====================================================================================
    ...
    weight = q1(FitStart:FitEnd).^2.*gaussmf(q1(FitStart:FitEnd),[0.55 -0.75]);  
...
dlmwrite('Structures2Average_SRIIHTRII_BS2.txt',TempTest, '\t');
...
plot(q1(FitStart:FitEnd),q1(FitStart:FitEnd).*transpose(ExptData2),'o','LineWidth',0.5,'MarkerSize',4,'color','#EDB120');
hold on; 
