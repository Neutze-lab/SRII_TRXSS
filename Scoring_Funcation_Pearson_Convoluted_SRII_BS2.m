...
% Read in experimental data from Richard's analysis.       
        SRIIold = load("Data\SRII_BS_2.mat");
...
% Read in Solvent Exclusion term from Solvent_Correction_Gromacs.m
         TEMP10 = load('Data\SolventExclusionFactorSRII.mat'); 
...
for kk = 1:size(stepnr,2)    
        filename1 = ["Data\SrII-grid3\input_fit\SrII-190BOG-301\input_fit\SrII-190BOG-301-rest-mem-rest-prot\SrII-190BOG-301-rest-mem-rest-prot"] ; 
        filename2 = ['Data\SrII-grid3\input_fit\SRII-190BOG-' num2str(stepnr(kk)) '\input_fit\SRII-190BOG-' num2str(stepnr(kk)) '-exc-mem-exc-prot\SRII-190BOG-' num2str(stepnr(kk)) '-exc-mem-exc-prot'] ;
        filename3 = ['Data\SrII-grid3\input_fit\SRII-190BOG-' num2str(stepnr(kk)) '\input_fit\SRII-190BOG-' num2str(stepnr(kk)) '-exc-mem-rest-prot\SRII-190BOG-' num2str(stepnr(kk)) '-exc-mem-rest-prot'] ;  
        filename4 = ['Data\SrII-grid3\input_fit\SRII-190BOG-' num2str(stepnr(kk)) '\input_fit\SRII-190BOG-' num2str(stepnr(kk)) '-rest-mem-exc-prot\SRII-190BOG-' num2str(stepnr(kk)) '-rest-mem-exc-prot'] ;            
...
% Check to see if it is an improvement, and if so then save values. 
            if  Rfactor(kk) < BestYet; 
                    ...
                    BestProtOnly = x1(1)*(dProtVacInterp(:,kk)).*Contrast;
                    BestIndex = kk;       
